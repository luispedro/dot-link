...
\begin{figure}
\centering
\includegraphics[width=.45\textwidth]{figures/mississippi-0}
...
We say that the string $s$ matches the string $t$ at position $p$ with $k$ errors if we can make $k$ substitutions in $s$ to obtain $s'$ which is a substring of $t$ at position $p$.
\end{definition}
...
Suffix trees are normally implemented so that each node consists of only two indices into the original string which are the start and end position of its incoming edge. Figure~\ref{fig:mississippi-nodes} illustrates this.
