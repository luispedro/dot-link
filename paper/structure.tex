...
For example, in the above mentioned node $\overline{issi}$, the error tree is formed by taking the strings starting at positions $\{7, 10\}$ (ie \putstring{sipi\$} and \putstring{i\$}) in a Patricia tree.
...
\begin{figure} 
\includegraphics[width=\textwidth]{figures/mississipi-1.eps}
% mississipi-1.eps: 300dpi, width=6.08cm, height=1.63cm, bb=0 0 718 193
\caption{1-error dotted tree for \putstring{mississipi}}%
\label{fig:mississipi-1}
\end{figure}


The one-error dotted tree for \putstring{mississipi} is shown in Figure~\ref{fig:mississipi-1}. The nodes are connected to their error trees by thick diagonal links.
