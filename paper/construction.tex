...
\begin{figure}
\centering
\includegraphics[width=.5\textwidth]{merge}
...
The construction of the tree using either Ukonnen or McCreight's algorithm assures one that this operation is always correct due to the way that the indices are created.
...
The analysis for the time cost above remains valid. Taking $N_i$ to mean the number of nodes of the i-error tree, we now have that the time cost is $\mathcal{O}(N_1|\Sigma|+N_2|\Sigma|+\dots+N_k|\Sigma)=\mathcal{O}((N_1+N_2+\dots+N_k)|\Sigma|)=\mathcal{O}(N_k|\Sigma|)=\mathcal{O}(N|\Sigma|)$ since $N_k \le N_i$ ($N_k = N$).
